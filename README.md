...
<li>
  $$
  \Delta I(\nu) = I(\nu) - I_{0}(\nu)
    = \frac{2h}{c^2}\int_{-\infty}^{\infty}P_1(s)ds(\frac{\nu_{0}^{3}}{e^{h\nu_0/k_BT_{rad}} -1} - \frac{\nu^{3}}{e^{h\nu/k_BT_{rad}} -1})
  $$
</li>
...
\begin{figure}[h]
    ...
    \caption{tSZE + kSZE for subclusters B and C}%
    \label{fig:example}%
\end{figure}
\FloatBarrier
...
<table>
  ...
  <figcaption> **Figure 6:** tSZE + kSZE for subclusters A, B, C, and D.</figcaption>
</table>
...
<h2 style="text-align: center;">The black data points provide the average brightness measured by NIKA in the 22 arcsec radius regions. Possible point source residuals were masked when computing the average surface brightness. [6] </h2>
